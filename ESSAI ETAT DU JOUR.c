...
typedef struct
{
    ...
    int montantVente;
    char nomV[15], prenomV[20], telV[10], loginV[6], num[15];
}VENTE;
typedef struct
{
    int nbrV, montV;
...
void modifUser(char nom[], FILE *f); //C'EST UN MENU QUI PROPOSE L'AJOUT ET LE BLOCAGE D'UTILISATEUR
...
int verifloginETmdp(char login[], char mdp[], char nom[], FILE *f);
...
FILE* vente(USER, char nom[], FILE*, FILE *f2, FILE *f3);
...
FILE* addetatToday(char nom[],int nbrV, int montV);

main()
{
    FILE *f, *f2, *f3, *f4, *f5, *f6;
//    listeCat(nomCat,addCat(nomCat,3));
//    listeProd(nomProd, addProd(nomProd,saisiePositive("PRODUITS"),nomCat,f2));
//    modifStock("1234567",nomProd,f);
//listeProd(nomProd,supProd("1234567",nomProd,f,nomSupp,f2));
//        listeUser(nomUser, addUser(nomUser));
//    listeUser(nomUser, unblockUser("JULES",nomUser,f));
//listeUser(nomUser, passer123("JULES",nomUser,f));
//modifUser(nomUser,f);
//modifProd(nomProd,f,nomCat,f2, nomSupp, f3);
//listeProd(nomProd,f);
...
//DE LUSER
{
    f = fopen(nom,"rb");
    USER u;
    char login[6], mdp[15], choix;//, tel[10], nom[15], prenom[20];
    int c, cpt,i;
        F //MENU DE CONNEXION
        printf("\t\t\t\t\t====================================================================================\n");
...
//        while(fread(&u,sizeof(u),1,f) != 0)
//        {
//            if(strcmp(u.login, login) == 0 && strcmp(u.mdp, mdp) == 0){
//                cpt = 1;
//                break;
//            }
//        }
...
FILE* vente(USER u, char nom[], FILE *f, FILE *f2, FILE *f3)
{
    ...
    char rep[3], rep2[3], nomfich[100], saveNomfich[100], code[7], etat[15];
    f = fopen(nom, "rb+");
    if(f)
    {
        do{
                        ...
                        else
                        {
                            if(qte>a.qte) //LA DEMANDE SUPERIEURE A LOFFRE
                            {
                                ...
                                if(strcmp(rep2,"oui")==0)
                                {
                                    ...
                                    {
                                        v.idVente = ++id_vente;
//                                        sprintf(etat,"ETAT_%d%d%d.txt", v.heureVente.a, v.heureVente.m, v.heureVente.j);
                                        sprintf(v.num, "%d%d%d%d%d%d", v.heureVente.j, v.heureVente.m, v.heureVente.a, v.heureVente.h, v.heureVente.mn, v.heureVente.s);
                                        sprintf(nomfich,"C:\\Users\\mkane\\OneDrive\\Bureau\\COURS ISI\\LANGAGE C\\MON EXAM\\BILLS\\%s_%s_%d_%s.txt","RECU",v.num,v.idVente,v.loginV);
                                        f2=fopen(nomfich,"w");
                                        if(f2)
                                        {
                                            fprintf(f2,"%d--------------------%s\n", v.idVente, v.num);
...
//                                        if(f4)
//                                        {
//                                            fprintf(f4,"Montant total : %d FCFA - Nombre d'articles vendu %d",v.montantVente,v.articleVente.qte);
//                                            fclose(f4);
//                                        }
                                    }
                                    else
